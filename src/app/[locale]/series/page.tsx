...
async function Series({ params: { locale } }: { params: { locale: string } }) {
  const filmService = new FilmService();
  const singleFilm = await filmService.getFilmSeries({ params: { page: 1 } });
  ...
  return (
    <Fragment>
      <Breadcrumbs data={[{ label: "Series", value: "series" }]} />
